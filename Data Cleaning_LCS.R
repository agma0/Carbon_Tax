...
household_information <- fread("LCS/household_information_sa")
expenditure_information <- fread("LCS/expenditure_information_sa")
...
check_2 <- subset(expenditure_information, (hh_id %in% hh_duplicates_expenditures_2$hh_id)) %>%
  pivot_wider(names_from = "item_code", values_from = "expenditures_year") # no duplicates - 418 hh spend same amount on one item - not 100% duplicate (checked manually)
...
# 5.1. Exchange Rates & CPI-Adjustment ####
...
# CPI-Adjustment (Inflation/Deflation) -> already in expenditure data included !!!


# 5.2   Matching GTAP Concordance ####
...
# 5.3   Matching Category Concordance ####
...
# 5.5   Vector with Carbon Intensities ####
...
rm(exchange.rate,  basic_household_information)
...
write_csv(final_incidence_information, sprintf("TBD/LCS/Carbon_Pricing_Incidence_%s.csv",  Country.Name))
write_csv(household_information,       sprintf("TBD/LCS/household_information_%s.csv", Country.Name))
write_csv(left_join(expenditures_fuels, expenditure_information_2), sprintf("TBD/LCS/fuel_expenditures_%s.csv", Country.Name))
