...
household_information <- house %>%
  ...
  rename (hh_id=UQNO, hh_size=hhsize, hh_weights=hholds_wgt, gender_hhh=SexOfHead, ethnicity_hhh=PopGroupOfHead, province=province_code, urban_1=SETTLEMENT_TYPE, 
          electricitycon=Q524ELECT, electricitymains= Q525AMAINS, electricitysource=Q525BSOURCE, electricityfree = Q526FREEELCET, cookingsource = Q527COOK, 
          lightingsource = Q527LIGHT, heatingsourcewater = Q527WATER, heatingsourcespace = Q527SPACE, income=income, consumption=expenditure) %>%
...
householdmembers <- person %>%
  ...
  rename (hh_id = UQNO, perno = PERSONNO, age = Q14AGE, relationhhh = Q16RELATION, education_hhh = Q21HIGHLEVEL)
...
household_members1 <- merge(hhh, assets, by ="hh_id", all=T)
household_information_sa <- merge(household_information, household_members1, by ="hh_id", all = TRUE)
...
fwrite(household_information_sa, "C:/Users/agath/Desktop/Masterthesis/Data/LCS/household_information_sa")
...
# write new table
fwrite(expenditure_information, "LCS/expenditure_information_sa")
